...
func NewAllowlist(items []string) (*Allowlist, error) {
	...
	for _, it := range items {
		...
		if strings.Contains(it, "/") {
			...
			if err != nil { return nil, fmt.Errorf("invalid CIDR %q: %w", it, err) }
			al.entries = append(al.entries, cidrEntry{net: ipnet})
			continue
		}
		// host:port
		h, p, ok := strings.Cut(it, ":")
		if !ok { return nil, fmt.Errorf("invalid allowlist item %q (want host:port or CIDR)", it) }
		port, err := strconv.Atoi(p)
		if err != nil || port <=0 || port > 65535 { return nil, fmt.Errorf("invalid port in %q", it) }
...
func (a *Allowlist) Allowed(host string, port int) bool {
	...
	for _, e := range a.entries {
		if ce, ok := e.(cidrEntry); ok {
			if ip != nil && ce.net.Contains(ip) { return true }
		}
		if e.Allowed(host, port) { return true }
	}
	return false
}
...
type hostPortEntry struct{ host string; port int }
