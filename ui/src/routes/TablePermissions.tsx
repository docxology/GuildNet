    ...
    <div class="space-y-4">
      ...
      <Show when={schema() && Array.isArray(schema()!.schema)}>
        <div class="text-xs p-3 border rounded bg-neutral-50 dark:bg-neutral-800">
          ...
          <div class="mt-1 text-[10px] text-neutral-500">
            Viewer/Editor roles will see masked values as ***
          </div>
        </div>
      </Show>
      <div class="space-y-2 border rounded p-3 bg-white dark:bg-neutral-900">
        <div class="grid gap-2 md:grid-cols-3">
          ...
          <div class="flex gap-2">
            ...
            />
            <button
              class="text-xs border rounded px-2"
              onClick={() => setScope(`db:${params.dbId || ''}`)}
            >
              db
            </button>
            <button
              class="text-xs border rounded px-2"
              onClick={() => setScope(`table:${params.table || ''}`)}
            >
              table
            </button>
          </div>
      ...
      <div class="border rounded bg-white dark:bg-neutral-900 text-sm">
        ...
        <Show when={(perms() || []).length === 0}>
          <div class="px-4 py-2 text-xs text-neutral-500">No permissions</div>
        </Show>
      </div>
    </div>
  )
