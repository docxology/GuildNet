...
export default function Launch() {
  ...
  const [presets, setPresets] = createSignal<Array<{ label: string; value: string; description?: string }>>([]);
  ...
  createEffect(async () => {
    ...
    setPresets(list.map(i => ({ label: i.label, value: i.image, description: i.description })));
  });
          ...
          <div class="space-y-3">
            <label class="block text-sm">Image
              <div class="mt-1 grid grid-cols-1 gap-2">
                <select class="w-full rounded-md border px-3 py-2 bg-white dark:bg-neutral-900"
                  value={image()}
                  onChange={async (e) => {
                ...
                <Input value={image()} onInput={(e) => setImage(e.currentTarget.value)} placeholder="…or enter a custom image (e.g., ghcr.io/org/app:tag)" />
              </div>
            </label>
            ...
            <div>
              <button class="inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium border bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700" onClick={() => setShowAdvanced(!showAdvanced())}>
