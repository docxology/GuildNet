...
async function fetchDatabases(): Promise<DatabaseInstance[]> { return listDatabases() }
...
export default function Databases() {
  ...
  const submit = async () => {
    if (!name().trim()) return
    setCreating(true)
    await createDatabase({ name: name().trim(), description: desc().trim() || undefined })
    setCreating(false)
    setOpen(false)
    setName(''); setDesc('')
  refetch()
  ...
  return (
    <div class="space-y-4">
      ...
      <div class="border rounded-lg divide-y overflow-hidden bg-white dark:bg-neutral-900">
        <For each={dbs()}>{(d) => (
          <a href={`/databases/${encodeURIComponent(d.id)}`} class="flex items-center px-4 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-800">
            <div class="flex-1">
      ...
      <Modal title="Create Database" open={open()} onClose={() => setOpen(false)} footer={
        <>
          <Button onClick={() => setOpen(false)} class="!bg-neutral-100 dark:!bg-neutral-800">Cancel</Button>
        ...
        <div class="space-y-4">
          ...
          <p class="text-xs text-neutral-500">A single multi-tenant DB is used in this MVP; creation updates metadata only.</p>
        </div>
      </Modal>
    </div>
  )
