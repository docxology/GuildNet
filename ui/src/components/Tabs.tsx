    ...
    <div>
      <div role="tablist" class="flex gap-2 mb-3 border-b">
        <For each={props.tabs}>{(t: { id: string; label: string }) => (
          <button type="button" role="tab" aria-selected={val() === t.id} class={`inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium border bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700 ${val() === t.id ? 'border-b-2 border-brand-600 text-brand-600' : ''}`} onClick={() => change(t.id)}>
            {t.label}
          </button>
        )}</For>
      </div>
